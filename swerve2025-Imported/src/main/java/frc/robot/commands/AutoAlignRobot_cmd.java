...
public class AutoAlignRobot_cmd extends Command {
  ...
  public AutoAlignRobot_cmd(CommandSwerveDrivetrain subsystem, robotVision_SS visionSubsystem) {
  ...
  double skewToleranceAtPoint3 = 6;
  double skewToleranceAtPoint2 = 4;
  double skewToleranceAtPoint1 = 1.5;
  ...
  double point3Distance = 0.8;
  double point2Distance = 0.65;
  double point1Distance = 0.55;
  double finalDistance = 0.22;
  ...
  //Rotation
  double skewSpeedMultiplierAtPoint3 = 1.1;
  double skewSpeedMultiplierAtPoint2 = 0.9;
  double skewSpeedMultiplierAtPoint1 = 0.8;
  ...
  double distanceFinalSpeedMultiplier = 0.8;
  ...
  @Override
  public void execute() {
    while ((alignRobot == false)&&(count <= 10000)) {
    targetPose = limelightTable.getEntry("camerapose_targetspace").getDoubleArray(new double[0]);
    if (targetPose.length != 0) {
      if (idSet == false) {
    ...
    drivetrain.setControl(driveRequest.withVelocityX(distanceVelocity).withVelocityY(txVelocity).withRotationalRate(skewVelocity));
    count++;
  }
  if (count > 10000) {
    System.out.println("Align Timed Out!");
    alignRobot = true;
  }
  }
